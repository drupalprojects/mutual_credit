...
$all_values = $lines = $line_styles = $line_colors = array();
...
//apply scale and axis markers only if we have some values!
if (count($all_values) > 1) {
  $max = max($all_values);
  $min = min($all_values);
  if ($max == $min) {//which would produce a horizontal line
    //get some other relevant limits
    //user the account limits
    $limits = user_limits($account);
    foreach ($limits as $limit){$maxes[] = $limit['max'];$mins[] = $limit['min'];}
    $max = max($maxes);
    $min = min($mins);
  }
